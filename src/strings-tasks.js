...
function countVowels(str) {
  ...
  return [...str].reduce((acc, curr) => acc + vowels.includes(curr), 0);
}
...
function isPalindrome(str) {
  ...
  const midStr = Math.floor(newStr.length / 2);

  const startStr = newStr.slice(0, midStr);
  const endStr = newStr.slice(newStr.length - midStr);

  return startStr === endStr.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  ...
  const arrSort = arr.sort((a, b) => b.length - a.length);
...
function extractNameFromTemplate(value) {
  return value.replace('!', '').split(',')[1].trimStart();
}
...
function encodeToRot13(str) {
  return [...str]
    .reduce((acc, curr) => {
      if (/[A-Za-z]/.test(curr)) {
        const newLetter = curr.charCodeAt(0) + 13;

        if (curr >= 'A' && curr <= 'Z') {
          if (newLetter > 90) {
            acc.push(String.fromCharCode(newLetter - 26));
          } else {
            acc.push(String.fromCharCode(newLetter));
          }
